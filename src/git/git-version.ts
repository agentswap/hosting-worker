export class GitVersion {
  private readonly major: number = Number.NaN
  private readonly minor: number = Number.NaN
  private readonly patch: number = Number.NaN
  ...
  checkMinimum(minimum: GitVersion): boolean {
    ...
    if (this.major === minimum.major) {
      ...
      // Minor is equal
      if (
        this.minor === minimum.minor && // Patch is insufficient
        this.patch &&
        this.patch < (minimum.patch || 0)
      ) {
        return false
      }
    }
